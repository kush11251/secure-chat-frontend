    ...
    <div class="w-full max-w-sm rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
      <div class="mb-4 flex items-center gap-2">
        ...
        <div>
          ...
          <div class="text-xs text-neutral-500">Create your account</div>
        </div>
      </div>
      ...
      <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-3">
        <div>
          ...
          <input formControlName="name" type="text" class="mt-1 w-full rounded-md border border-neutral-300 p-2 focus:outline-none focus:ring-2 focus:ring-brand-500" placeholder="Your name" />
        ...
        <button class="w-full rounded-md bg-brand-600 px-4 py-2 text-white hover:bg-brand-700 disabled:opacity-60" type="submit" [disabled]="loading() || form.invalid">{{ loading() ? 'Creating…' : 'Create account' }}</button>
